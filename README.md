...
[cdc.gov](www.cdc.gov/) (link from slide) 
...
The rows represent country/year data points, allowing cross referencing by country. The **datasets were merged** and cleaned for exploratory visualization. The **top 25 countries** by zoonotic cases were then extracted from the merged dataset, and a **linear regression** was run to examine correlation. Note: More detailed explanations are available in the slideshow linked above, but GitHub does not render charts from annotated Python notebooks.
